...
public class Advancepay extends Activity implements JsonResponse, View.OnClickListener {
...
//
//
					final CharSequence[] items = {"From Wallet","From Card","Cancel"};

					AlertDialog.Builder builder = new AlertDialog.Builder(Advancepay.this);
...
//
//                }
							if (items[item].equals("From Wallet")) {

								JsonReq jr= new JsonReq();
								jr.json_response=(JsonResponse)Advancepay.this;
								String q="/advancew?amt=" + amt+"&bookid="+REQUEST_STATUS.bid+"&total="+REQUEST_STATUS.val+"&loginid="+Login.logid+"&aprid="+REQUEST_STATUS.apr_id;
								jr.execute(q);
//
							}
							else if (items[item].equals("From Card")) {

								JsonReq jr= new JsonReq();
								jr.json_response=(JsonResponse)Advancepay.this;
								String q="/advance?amt=" + amt+"&bookid="+REQUEST_STATUS.bid+"&total="+REQUEST_STATUS.val+"&aprid="+REQUEST_STATUS.apr_id;
								jr.execute(q);
//
							}
...
//                }
	...
	public void response(JSONObject jo) {
		...
		try{
			...
			{
				e.setError("Payment failed Plaese Check Your Wallet Or Try Again ");
            	e.setFocusable(true);
			}
		}
